...
while [ $j -gt $k ]; do
    ...
    if [ $? -eq 0 ];then
        ...
        hadoop jar /work/recommender/remd-hadoop-1.0-SNAPSHOT-jar-with-dependencies.jar RecommendCleaner /recommender/tmp/recommender.log.$stat_date /recommender/result/${stat_before}/ /recommender/result_temp/${stat_date}/ /recommender/result/${stat_date}/
    else
